...
const Content = () => {
    return (
        <div style={contentStyle}>
           <h1 style={headingStyle}>Java Lists:</h1> 
           ...
           <p style={baseTextStyle}>Singly linked lists are efficient for insertion and deletion of elements at the beginning or end of the list but less efficient for accessing elements at arbitrary positions.</p>

           <h1 style={headingStyle}>Doubly Linked List:</h1>
           <p style={baseTextStyle}>A doubly linked list is a data structure similar to a singly linked list, but it has one key difference: each node has references to both the next and the previous nodes. This "doubly" linked structure allows traversal in both directions, from head to tail and from tail to head. Here are the main characteristics of a doubly linked list:</p> 
           <ul style={{ ...baseTextStyle, listStylePosition: 'inside', padding: 0 }}>
           ...
           <p style={baseTextStyle}>Doubly linked lists are more versatile than singly linked lists because they allow efficient traversal in both directions. However, they consume more memory due to the additional references in each node. They are often used when you need bidirectional traversal or frequent insertion and deletion at both ends of the list.</p> 
        </div>
    );
}
